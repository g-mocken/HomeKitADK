...
int main(int argc, char* _Nullable argv[_Nullable]) {
	...
	for (i = 1; i < argc; i++) {
		...
		char * ptrName = strtok((char *) argv[i], delimiter);
		char * ptrValue = strtok((char *) NULL, delimiter);

		if (strcmp(ptrName, "debug") == 0) {
			//debugMode = true;
			(void) ptrValue; // supress warning
			continue; // we are not interested in an assigned value in this case
		}

		// further strcmp() could check for other arguments here

		// or check for "NAME=1234" key-value pairs (space separated, like all CLI arguments)
	...
	HAPLogInfo(&kHAPLog_Default, "%s: Version of %s / %s\n", __func__, __DATE__, __TIME__);
